...
QHexView::QHexView(QWidget *parent)
  ...
  m_charWidth =
      fontMetrics().horizontalAdvance(QString("1234567890abcdef")) / 16;
#else
...
void QHexView::paintEvent(QPaintEvent *event) {
  ...
  painter.fillRect(QRect(m_posAddr, event->rect().top(), m_posHex, height()),
                   addressAreaColor);
  ...
  QBrush selected = QBrush(Qt::yellow);
  // QBrush(QColor(0x6d, 0x9e, 0xff, 0xff));
  QByteArray data =
      m_pdata->getData(firstLineIdx * m_bytesPerLine,
  ...
  for (int lineIdx = firstLineIdx, yPos = yPosStart; lineIdx < lastLineIdx;
    ...
    painter.drawText(m_posAddr + m_charWidth, yPos, address);

    for (int xPos = m_posHex + m_charWidth, i = 0;
         i < m_bytesPerLine &&
      ...
      QString val = QString::number(
          (data.at((lineIdx - firstLineIdx) * m_bytesPerLine + i) & 0xF0) >> 4,
          16);
    ...
    for (int xPosAscii = m_posAscii + m_charWidth, i = 0;
         ((lineIdx - firstLineIdx) * m_bytesPerLine + i) < data.size() &&
         (i < m_bytesPerLine);
  ...
  if (hasFocus()) {
    ...
    painter.fillRect(cursorX, cursorY, 2, m_charHeight,
...
void QHexView::ensureVisible() {
  ...
  if (cursorY < firstLineIdx)
    ...
    verticalScrollBar()->setValue(cursorY - areaSize.height() / m_charHeight +
                                  1);
}
